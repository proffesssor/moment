...
window.addEventListener('load', function() {
  ...
  setTimeout(()=>{
    document.querySelector('.promo').classList.remove('none');
    document.querySelector('.close-btn').addEventListener('click', (e)=>{
      e.preventDefault();
      document.querySelector('.promo').classList.add('none');
    });
  }, 3000);
...
$(document).ready(function() {
	$('.book__form').submit(function() { // проверка на пустоту заполненных полей. Атрибут html5 — required не подходит (не поддерживается Safari)
...
$(document).ready(function() {
	$('.call__wrapper').submit(function() { // проверка на пустоту заполненных полей. Атрибут html5 — required не подходит (не поддерживается Safari)
...
$(function($){
  ...
  // $('input[name="phone__num"]').mask("+7(999) 999-9999");
});